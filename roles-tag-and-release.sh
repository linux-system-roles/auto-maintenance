...
view_diffs() {
    ...
    while [ "$action" != "q" ]; do
        ...
        for hsh in $(git log --pretty=format:%h --no-merges --reverse "${tag}.."); do
            git show --stat "$hsh"
            read -p 'View full diff (y)? Start over (s)? Quit (q)? ' action
            if [ "$action" = y ]; then
                git show "$hsh"
...
read -p 'Examine role? ' examine_role
if [ "${examine_role:-n}" = y ]; then
    ...
    # see the changes?
    read -p 'View changes? ' view_changes
    if [ "${view_changes:-n}" = y ]; then
        view_diffs "${latest_tag}"
    fi

    read -p "Old tag is $latest_tag - what is the new tag? " new_tag
    if [ -n "${new_tag}" ]; then
        read -p 'Edit release notes? ' rel_notes
        if [ "${rel_notes:-n}" = y ]; then
            ...
            read -p 'Create new github release? ' create_release
            if [ "${create_release:-n}" = y ]; then
                hub release create -t "$mainbr" -F "$rel_notes_file" "$new_tag"
                read -p 'Publish to Galaxy? ' publish_galaxy
